...
ANIMATION_RIGHT = [('%s/mario/r3.png' % ICON_DIR),
            ('%s/mario/r3.png' % ICON_DIR),
            ('%s/mario/r3.png' % ICON_DIR),
            ('%s/mario/r3.png' % ICON_DIR),
            ('%s/mario/r3.png' % ICON_DIR)]
ANIMATION_LEFT = [('%s/mario/l3.png' % ICON_DIR),
        ...
        for p in platforms:
            if sprite.collide_rect(self, p): # если есть пересечение платформы с игроком
                ...
                if isinstance(p, blocks.BlockDie): # если пересакаемый блок - blocks.BlockDie или Monster
                       p.image = image.load("%s/blocks/dieBlockBoom.png" % ICON_DIR)
                       time.wait(200)
                       self.teleporting(self.startX, self.startY)
                       #self.die()# умираем
                elif isinstance(p, monsters.Monster):
                       time.wait(200)
                       self.die()
